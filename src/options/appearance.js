...
function createAppearanceController(options = {}) {
	...
	const appearanceApi = options.appearanceApi;
	...
	const previewTranslation = doc.getElementById("reading-preview-translation");
	...
	function readTheme(fieldMap) {
		return Object.fromEntries(
			Object.entries(fieldMap).map(([key, id]) => [key, getElement(id).value]),
		);
	}
	...
	function renderPreview() {
		...
		previewTranslation.classList.toggle("has-fade", inline.enableFadeAnimation);
		...
		selectionPreviewLanguage.textContent = targetLanguage || "Target language";
...
export { createAppearanceController };
export default { createAppearanceController };
