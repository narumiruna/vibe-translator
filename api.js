(function (root) {
  ...
  const TECH_TERM_REGEX = /\b(?:API|LLM|OpenAI|GitHub|Telegram|Markdown|README|Chrome|Yahoo Finance|JSON|HTML|CSS|JavaScript|TypeScript|Node\.js)\b/g;
  ...
  function extractTokensForText(text, tokens) {
    return (tokens || []).filter((token) => String(text || '').includes(token.placeholder));
  }
    ...
    for (const item of items || []) {
      ...
      if (parts.length <= 1) {
        expandedItems.push({
          ...
          text: parts[0] ? parts[0].text : normalizeChunkText(normalizedItem.maskedText),
          sourceId: item.id,
          partIndex: 0,
          partCount: 1,
          joiner: '',
          protectedFragments: extractTokensForText(normalizedItem.maskedText, normalizedItem.protectedFragments)
        });
      ...
      for (let index = 0; index < parts.length; index += 1) {
        ...
        expandedItems.push({
          ...
          protectedFragments: extractTokensForText(parts[index].text, normalizedItem.protectedFragments)
        });
        partIds.push(partId);
      }
    ...
    for (const translation of translations || []) {
      ...
      for (const fragment of sourceItem.protectedFragments || []) {
        if (!translation.translation.includes(fragment.placeholder)) {
          throw new Error(`Protected placeholder missing from translation for ${translation.id}`);
        }
      }
    }
  }
  ...
  async function requestTranslationsBatched(options) {
    ...
    const fetchImpl = options.fetchImpl || root.fetch;
    const concurrency = Math.max(1, Math.floor(options.concurrency || DEFAULT_MAX_CONCURRENCY));
  ...
  async function requestTranslationsBatchedProgressive(options) {
    ...
    const fetchImpl = options.fetchImpl || root.fetch;
    const concurrency = Math.max(1, Math.floor(options.concurrency || DEFAULT_MAX_CONCURRENCY));
    const onChunkResolved = typeof options.onChunkResolved === 'function'
      ? options.onChunkResolved
      : null;
    const onChunkRejected = typeof options.onChunkRejected === 'function'
  ...
  function createProgressiveMergeState(plan) {
    return {
      ...
      expandedById: new Map((plan.expandedItems || []).map((item) => [item.id, item]))
    };
  }
  ...
  function consumeProgressiveTranslations(plan, state, translations) {
    ...
    for (const sourceId of touchedSegmentIds) {
      ...
      const hasAllParts = group.partIds.every((partId) => state.partTranslations.has(partId));
  ...
  function mergeRecursiveTranslations(plan, translations) {
    ...
    const expandedById = new Map((plan.expandedItems || []).map((item) => [item.id, item]));
    const merged = [];
